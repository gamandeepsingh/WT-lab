...
const Poster = ({ formData, file }) => {
  ...
  const getFormDate = () => {
    switch (formData.DateType) {
      case "Single":
        return formData.Date ? formatDate(formData.Date) : "";
      case "Multi":
        if (!formData.StartDate || !formData.EndDate) return "";
        return startToEndDateFormat(formData.StartDate, formData.EndDate);
      case "Recurring":
        return formData.RecurringPattern === "Specific Day"
            ? `Every ${formData.RecurringDay || ""}`
            : "Every Weekend";
      default:
        return formData.Date ? formatDate(formData.Date) : "";
    }
